...
void UKPCLDefaultRCO::Server_DoSkill_Once_Implementation( AKPCLLevelingSubsystem* Subsystem, EOnceSkilled Skilling, UKPCLLevelingSkillInformation* SkillInformation, AFGCharacterPlayer* Char )
{
	if( SkillInformation && Subsystem )
	{
		Subsystem->DoSkill_Once( Skilling, SkillInformation, Char );
		Subsystem->ForceNetUpdate();
	}
}

void UKPCLDefaultRCO::Server_DoSkill_Default_Implementation( AKPCLLevelingSubsystem* Subsystem, ESkillingDefaults Skilling, UKPCLLevelingSkillInformation* SkillInformation, AFGCharacterPlayer* Char )
{
	if( SkillInformation && Subsystem )
	{
		Subsystem->DoSkill_Default( Skilling, SkillInformation, Char );
		Subsystem->ForceNetUpdate();
	}
}

void UKPCLDefaultRCO::Server_LEVELUP_Implementation( AKPCLLevelingSubsystem* Subsystem, AFGPlayerState* PlayerState, FKPCLLevelStepInformation LevelInformation )
{
	if( Subsystem && PlayerState )
	{
		Subsystem->LEVELUP( PlayerState, LevelInformation );
		Subsystem->ForceNetUpdate();
	}
}

void UKPCLDefaultRCO::Server_AddSkillExp_Implementation( AKPCLLevelingSubsystem* Subsystem, int Points, AFGPlayerState* PlayerState, EExpType Type )
{
	if( Subsystem && PlayerState )
	{
		Subsystem->AddSkillExp( Points, PlayerState, Type );
		Subsystem->ForceNetUpdate();
	}
}

void UKPCLDefaultRCO::Server_AddPending_Implementation( AKPCLLevelingSubsystem* Subsystem, AFGPlayerState* PlayerState )
{
	if( Subsystem && PlayerState )
	{
		Subsystem->AddPending( PlayerState );
		Subsystem->ForceNetUpdate();
	}
}

void UKPCLDefaultRCO::Server_DoReskill_Implementation( AKPCLLevelingSubsystem* Subsystem, UKPCLLevelingSkillInformation* SkillInformation, ESkillingDefaults Skilling, AFGCharacterPlayer* Char )
{
	if( Subsystem && SkillInformation )
	{
		Subsystem->DoReskill( SkillInformation, Skilling, Char );
		Subsystem->ForceNetUpdate();
	}
}

void UKPCLDefaultRCO::Server_LocalExpGained_Implementation( AKPCLLevelingSubsystem* Subsystem, float Exp, EExpType Type, AFGPlayerState* PlayerState )
{
	if( Subsystem && PlayerState )
	{
		Subsystem->LocalExpGained( Exp, Type, PlayerState );
		Subsystem->ForceNetUpdate();
	}
}

void UKPCLDefaultRCO::Server_GlobalExpGained_Implementation( AKPCLLevelingSubsystem* Subsystem, float Exp, EExpType Type )
{
	if( Subsystem )
	{
		Subsystem->GlobalExpGained( Exp, Type );
		Subsystem->ForceNetUpdate();
	}
}

void UKPCLDefaultRCO::Server_DoOnceReskill_Implementation( AKPCLLevelingSubsystem* Subsystem, UKPCLLevelingSkillInformation* SkillInformation, EOnceSkilled Skilling, AFGCharacterPlayer* Char )
{
	if( Subsystem && SkillInformation )
	{
		Subsystem->DoOnceReskill( SkillInformation, Skilling, Char );
		Subsystem->ForceNetUpdate();
	}
}

void UKPCLDefaultRCO::Server_DoCheat_Implementation( AKPCLLevelingSubsystem* Subsystem, const FString& PlayerName, FKPCLCheating Cheat )
{
	if( Subsystem )
	{
		Subsystem->Cheat_SetData( PlayerName, Cheat );
		Subsystem->ForceNetUpdate();
	}
}
