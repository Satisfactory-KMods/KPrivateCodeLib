...
AKPCLNetworkBuildingBase::AKPCLNetworkBuildingBase() {
	...
	mNeedModDependencies = true;
	mModRefsToCheck = {FName("SatisfactoryPlus"), FName("KNetworkStorage")};
}
	...
	if(HasAuthority()) {
		if(GetNetworkConnectionComponent()) {
			...
			if(bBindNetworkComponent) {
				GetNetworkConnectionComponent()->OnConnectionChanged.AddUObject(this, &AKPCLNetworkBuildingBase::OnCircuitChanged);
				GetNetworkInfoComponent()->CoreStateChanged.AddUObject(this, &AKPCLNetworkBuildingBase::OnHasCoreChanged);
			...
			while(!mInventoryQueue.IsEmpty()) {
				...
				if(QueueItem.IsValid()) {
					if(QueueItem.mAddAmount) {
						UKBFLCppInventoryHelper::AddItemsInInventory(GetInventory(), QueueItem.mAmount.ItemClass, QueueItem.mAmount.Amount);
					} else {
