...
void AKPCLNetworkCore::onProducingFinal_Implementation()
{
	...
	UKBFLCppInventoryHelper::AddStackInInventory( GetFluidBufferInventory(), FInventoryStack( GetCurrentOutputAmount(), mOutputProduction.ItemClass ) );
...
void AKPCLNetworkCore::GetDismantleRefund_Implementation( TArray< FInventoryStack >& out_refund ) const
{
...
void AKPCLNetworkCore::GetCoreData( FCoreDataSortOptionStruc SortOption, TArray< FCoreInventoryData >& Data )
{
	...
	{
	case ECoreDataShowOption::fluid: for( FCoreInventoryData SlotMapping : mSlotMappingAll )
		{
		...
		break;
	case ECoreDataShowOption::solid: for( FCoreInventoryData SlotMapping : mSlotMappingAll )
		{
		...
		break;
	default: CoreData = mSlotMappingAll;
	}
	...
	for( FCoreInventoryData InventoryData : CoreData )
	{
		if( ensure( InventoryData.mItem ) )
		{
			...
			else if( ( SortOption.mFilteredUnlocked && HasRecipe && !SortOption.mNameFilter.IsEmpty() ) || !SortOption.mFilteredUnlocked )
			{
				if( InventoryData.mItem->GetName().Contains( *SortOption.mNameFilter ) || UFGItemDescriptor::GetAbbreviatedDisplayName( InventoryData.mItem ).ToString().Contains( *SortOption.mNameFilter ) || UFGItemDescriptor::GetItemName( InventoryData.mItem ).ToString().Contains( *SortOption.mNameFilter ) && HasRecipe )
				{
					Data.Add( InventoryData );
				}
			}
	...
	Data.Sort( [&]( const FCoreInventoryData& A, const FCoreInventoryData& B )
	{
		...
		{
		case ECoreDataSortOption::alphab: return UFGItemDescriptor::GetItemName( A.mItem ).ToString() < UFGItemDescriptor::GetItemName( B.mItem ).ToString();

		case ECoreDataSortOption::revalphab: return UFGItemDescriptor::GetItemName( A.mItem ).ToString() > UFGItemDescriptor::GetItemName( B.mItem ).ToString();

		case ECoreDataSortOption::form: return UFGItemDescriptor::GetForm( A.mItem ) < UFGItemDescriptor::GetForm( B.mItem );

		case ECoreDataSortOption::revform: return UFGItemDescriptor::GetForm( A.mItem ) > UFGItemDescriptor::GetForm( B.mItem );

		case ECoreDataSortOption::formalphab: return UFGItemDescriptor::GetItemName( A.mItem ).ToString() < UFGItemDescriptor::GetItemName( B.mItem ).ToString() && UFGItemDescriptor::GetForm( A.mItem ) < UFGItemDescriptor::GetForm( B.mItem );

		case ECoreDataSortOption::revformalphab: return UFGItemDescriptor::GetItemName( A.mItem ).ToString() > UFGItemDescriptor::GetItemName( B.mItem ).ToString() && UFGItemDescriptor::GetForm( A.mItem ) > UFGItemDescriptor::GetForm( B.mItem );

		case ECoreDataSortOption::revindex: return A.mInventoryIndex > B.mInventoryIndex;

		default: return A.mInventoryIndex < B.mInventoryIndex;
		}
	} );
}
...
void AKPCLNetworkCore::PostInitializeComponents()
{
	...
	{
		if( UKPCLNetworkConnectionComponent* NetworkConnectionComponent = Cast< UKPCLNetworkConnectionComponent >( PowerConnectionComponent ) )
		{
...
float AKPCLNetworkCore::GetBytesForItemClass( TSubclassOf< UFGItemDescriptor > itemClass, float Num )
{
	if( ensureAlways( itemClass ) && Num > 0 )
	{
		...
		{
			ByteSize = Num / AKPCLNetworkCore::mFluidItemsPerBytes;
		}

		else
		{
			ByteSize = Num / AKPCLNetworkCore::mSolidItemsPerBytes;
		}
...
int32 AKPCLNetworkCore::GetMaxItemsByBytes( TSubclassOf< UFGItemDescriptor > itemClass, float FluidBytes, float SolidBytes )
{
	if( ensureAlways( itemClass ) && ( FluidBytes > 0.f || SolidBytes > 0.f ) )
	{
		...
		if( Form == EResourceForm::RF_GAS || Form == EResourceForm::RF_LIQUID )
		{
			MaxItems = FMath::Floor( FluidBytes * AKPCLNetworkCore::mFluidItemsPerBytes );
		}

		else
		{
			MaxItems = FMath::Floor( SolidBytes * AKPCLNetworkCore::mSolidItemsPerBytes );
		}
...
float AKPCLNetworkCore::GetBytesForItemAmount( FItemAmount Amount, bool& IsFluid )
{
	if( ensureAlways( Amount.ItemClass ) && Amount.Amount > 0 )
	{
		...
		{
			IsFluid = true;
			return 1 / AKPCLNetworkCore::mFluidItemsPerBytes * Amount.Amount;
		}

		IsFluid = false;
		return 1 / AKPCLNetworkCore::mSolidItemsPerBytes * Amount.Amount;
	}
	return 0.f;
}
...
int32 AKPCLNetworkCore::GetCurrentInputAmount() const
{
	...
	return FMath::Max(FMath::Min( mMaxProduceAmount, FMath::TruncToInt( mInputConsume.Amount * TotalBytes ) ), mInputConsume.Amount );
}

int32 AKPCLNetworkCore::GetCurrentOutputAmount() const
{
	const float TotalBytes = ( mTotalSolidBytes + mTotalFluidBytes ) / mByteCalcDiv;
	return FMath::Max( FMath::Min( mMaxConsumeAmount, FMath::TruncToInt( mOutputProduction.Amount * TotalBytes ) ), mInputConsume.Amount );
}
...
bool AKPCLNetworkCore::CanProduce_Implementation() const
{
	...
	{
		return
			GetFluidBufferInventory()->HasItems( mInputConsume.ItemClass, GetCurrentInputAmount() ) &&
			UKBFLCppInventoryHelper::CanStoreItemStack( GetFluidBufferInventory(), FInventoryStack( GetCurrentOutputAmount(), mOutputProduction.ItemClass ) );
	}
...
void AKPCLNetworkCore::TickPlayerBufferInventory( float dt )
{
	...
	if( !GetPlayerBufferInventory() || !GetInventory() ) return;

	if( GetPlayerBufferInventory()->IsEmpty() ) return;
	...
	if( Solid > 0.0f || Fluid > 0.0f )
	{
		...
		if( GetPlayerBufferInventory()->GetStackFromIndex( BufferIndex, Stack ) )
		{
			if( FCoreInventoryData* CoreData = mSlotMappingAll.FindByKey( Stack.Item.GetItemClass() ) )
			{
				...
				if( Stack.HasItems() )
				{
					GetPlayerBufferInventory()->RemoveFromIndex( BufferIndex, GetInventory()->AddStackToIndex( ItemIdx, Stack, true ) );
				}
			}
		}
	}
...
void AKPCLNetworkCore::HandlePower( float dt )
{
	...
	if( IsValid( Network ) )
	{
		...
		GetPowerInfo()->SetMaximumTargetConsumption( IsProducing() ? FMath::Max( Stats.MaximumPowerConsumption, 0.1f ) : 0.1f );
	}
}
...
void AKPCLNetworkCore::ConfigureCoreInventory()
{
	...
	auto ModContent = AModContentRegistry::Get( GetWorld() );
	auto ItemDescriptors = ModContent->GetLoadedItemDescriptors();

	for( auto ItemDescriptor : ItemDescriptors )
	{
		if( TSubclassOf< UFGItemDescriptor > Item = TSubclassOf< UFGItemDescriptor >( ItemDescriptor.RegisteredObject ) )
		{
		...
		for( int i = 0; i < Items.Num(); ++i )
		{
			if( Items.IsValidIndex( i ) )
			{
				...
				if( ensure( Item ) )
				{
					...
					Data.mItemForm = Form == EResourceForm::RF_SOLID ? EResourceForm::RF_SOLID : EResourceForm::RF_LIQUID;
			...
			{
				UKPCLBlueprintFunctionLib::SetAllowOnIndex_ThreadSafe( GetInventory(), i, UFGNoneDescriptor::StaticClass() );
			}
		}
