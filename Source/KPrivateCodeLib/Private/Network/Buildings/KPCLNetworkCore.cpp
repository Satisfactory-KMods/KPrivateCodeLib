...
void                          AKPCLNetworkCore::GetDismantleRefund_Implementation(TArray<FInventoryStack>& out_refund, bool noBuildCostEnabled) const {
...
bool AKPCLNetworkCore::CanProduce_Implementation() const {
	...
	if(GetPowerInfo()->HasPower() && GetFluidBufferInventory()) {
		return GetFluidBufferInventory()->HasItems(mInputConsume.ItemClass, GetCurrentInputAmount()) && UKBFLCppInventoryHelper::CanStoreItemStack(GetFluidBufferInventory(), FInventoryStack(GetCurrentOutputAmount(), mOutputProduction.ItemClass));
	}
