...
UFGFactoryConnectionComponent* AKPCLProducerBase::GetConv(int Index, ECDirection Direction) const {
...
TArray<UFGFactoryConnectionComponent*> AKPCLProducerBase::GetAllConv(ECDirection Direction) const {
...
UFGPipeConnectionFactory* AKPCLProducerBase::GetPipe(int Index, ECDirection Direction) const {
...
TArray<UFGPipeConnectionFactory*> AKPCLProducerBase::GetAllPipes(ECDirection Direction) const {
...
void AKPCLProducerBase::CheckingModDependencies() {
	if(!HasAuthority()) {
		return;
	}

	if(mNeedModDependencies) {
		...
		if(ensureMsgf(ModLoadingLibrary, TEXT("Cannot find ModLoadingLibrary!!!"))) {
			bool Found = false;
			for(FName ModRefsToCheck: mModRefsToCheck) {
				if(ModLoadingLibrary->IsModLoaded(ModRefsToCheck.ToString())) {
					Found = true;
					break;
				}
			}
			...
			if(!Found) {
				...
				Execute_GetDismantleRefund(this, Stacks, false);
				AFGCrate* Crate;
				AFGItemPickup_Spawnable::SpawnInventoryCrate(GetWorld(), Stacks, GetActorLocation(), {this}, Crate);
...
void AKPCLProducerBase::InitAudioConfig() {
	UConfigPropertyFloat* Property = mAudioConfig.GetPropertyAsType();
	if(IsValid(Property)) {
...
void AKPCLProducerBase::OnAudioConfigChanged_Native() {
	for(FKPCLAudioComponent AudioComponent: mAudioComponents) {
		AudioComponent.SetVolumePercent(mAudioConfig.GetValue());
	}
